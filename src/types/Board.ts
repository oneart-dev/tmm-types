...
export class Board<SecurityDataType = unknown> extends HttpClient<SecurityDataType> {
  ...
  /**
   * @description Returns server preview data for a widget using the provided filters. Preferred tag filtering uses `tag_groups`, for example `tag_groups=[{"column":1,"ids":[10,11],"params":"all:"},{"column":2,"ids":[20],"params":"not:"}]`. Legacy `tags`/`exit_tags` remain supported but are deprecated.
   *
   * @tags dashboard
   * @name PreviewWidgetDetail
   * @summary Get Widget Preview Data
   * @request GET:/board/preview/widget/{id}
   * @secure
   */
  previewWidgetDetail = (
    id: number,
